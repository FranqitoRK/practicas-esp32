...
void setup() {
  ...
  xTaskCreatePinnedToCore(
    taskSemaphore,  // Función que vamos a ejecutar
    "Task_Core0",   // Nombre para la función (solo para debugging)
    4096,           // Tamaño del stack
    NULL,           // Parámetros
    1,              // Prioridad de la tarea
    NULL,           // No es importante
    0               // Asignamos el núcleo 0
  );
}

void taskSemaphore(void *parameter) {
  while (1) {
    int coreID = xPortGetCoreID();  // Detecta en qué núcleo estamos
    ...
    if (xSemaphoreTake(mySemaphore, portMAX_DELAY) == pdTRUE) {
      ...
      Serial.print("Core ");
      Serial.print(coreID);
      Serial.println(" ha liberado el semáforo");
...
void loop() {
  // No necesitamos hacer nada en el loop, ya que todo está en las tareas (RTOS)
}
